...
void *elf_hook(char const *module_filename, void const *module_address, char const *name, void const *substitution)
{
	LOGD("[+] elf_hook :\n\tmodule name: %s\n\tmodule address: 0x%x\n\tfunction to replace: %s",
			module_filename, module_address, name);
    ...
    *name_address = NULL;  //address of relocation for symbol named "name"
    ...
    if (descriptor < 0) {
		LOGE("[+] Can not open %s", module_filename);
        return original;
	}
    ...
    {  //if something went wrong
        ...
        free(symbol);
        close(descriptor);
		LOGE("[+] Something went wrong when finding tables");

        return original;
    }

	read_header(descriptor, &header);
	Elf_Half type = header->e_type;
/*
	Elf_Half shndx = symbol->st_shndx;

	if (shndx) {
        Elf_Shdr *sections = NULL;
        read_section_table(descriptor, header, &sections);
		Elf_Shdr target = sections[shndx];
		if (target.sh_type == SHT_PROGBITS) {
			const char* strings = NULL;
            read_string_table(descriptor, &sections[header->e_shstrndx], &strings);
			const char* name = &strings[target.sh_name];
			LOGD("Not in rel sections! In section: %s", name);
		}
	}
*/
...
//now we've got ".rel.plt" (needed for PIC) table and ".rel.dyn" (for non-PIC) table and the symbol's index
	LOGD("name_index: %d", name_index);
    for (i = 0; i < rel_plt_amount; ++i) { //lookup the ".rel.plt" table
		int ndx = ELF_R_SYM(rel_plt_table[i].r_info);
        if (ndx == name_index)  //if we found the symbol to substitute in ".rel.plt"
        {
		    LOGD("Symbol found. Current index: %d", ndx);
			...
			if (type == ET_DYN) {
                ...
                *(size_t *)(rel_plt_table[i].r_offset) = (size_t)substitution;  //and replace it with the substitutional
			}
...
//we will get here only with 32-bit non-PIC module
	LOGD("[+] 32-bit non-PIC module: fallback");
	
    for (i = 0; i < rel_dyn_amount; ++i)  {//lookup the ".rel.dyn" table
		int ndx = ELF_R_SYM(rel_dyn_table[i].r_info);
		LOGD("Current index: %d", ndx);
        if (ndx == name_index)  //if we found the symbol to substitute in ".rel.dyn"
        {
			LOGD("In .rel.dyn section, name index: %d", name_index);
